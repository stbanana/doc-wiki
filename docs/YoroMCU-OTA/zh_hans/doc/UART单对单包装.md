...
| 帧头(2字节) | 控制消息(4字节 小端32位) | 额外消息长度(小端2字节) | 额外消息(n字节)  | 和校验取反(1字节) |
| ----------- | ---------------- | ------------------- | ---------------- | ----------------- |
| FE EF       | **基础消息结构** | 包装                | **基础消息结构** | 包装              |
...
| 帧头(2字节) | 控制消息(4字节)  | 回复状态长度(2字节) | 回复状态(8字节)  | 和校验取反(1字节) |
| ----------- | ---------------- | ------------------- | ---------------- | ----------------- |
| FE EF       | **基础消息结构** | 包装(固定是00 08)   | **基础消息结构** | 包装              |
